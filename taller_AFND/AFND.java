...
public class AFND{
    ...
    private Set<Estado> funcionDeltaExtendida(final Estado q,final String cadena){
        Set<Estado> x = new HashSet<Estado>();
        if(cadena.length()==0)
        {
            x.add(q);
            eClausura(x);
        ...
        Character c = cadenaAux.charAt(cadena.length()-1);
        cadenaAux = cadenaAux.substring(0,cadenaAux.length()-2);
        x = funcionDeltaExtendida(q,cadenaAux);
        Set<Estado> y= new HashSet<Estado>();
        for(Estado e: x){
            y.addAll(funcionDelta(e,c));
        }
    ...
    public Boolean validarEntrada(final String w){
        ...
        Set<Estado> x = new HashSet<Estado>();
        x=funcionDeltaExtendida(inicial,w);
        Set<Estado> respuesta = finales;
        respuesta.retainAll(x);
        return respuesta.size() > 0;
